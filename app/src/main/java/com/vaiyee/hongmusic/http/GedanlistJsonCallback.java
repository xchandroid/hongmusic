...
public abstract class GedanlistJsonCallback<T> extends Callback<T> {
    private Class<T> clazz;
    private Gson gson;
    public GedanlistJsonCallback(Class<T> clazz)
    {
        this.clazz = clazz;
        gson = new Gson();
    }
    ...
    public T parseNetworkResponse(Response response, int id) throws Exception {
        try
        {
            ...
            String subjson = Jsonresponse.substring(Jsonresponse.indexOf("{"),Jsonresponse.lastIndexOf("}")+1);
            createjson(subjson);
            return gson.fromJson(subjson,clazz);
        }catch (Exception e)
        {
            e.printStackTrace();
        }
