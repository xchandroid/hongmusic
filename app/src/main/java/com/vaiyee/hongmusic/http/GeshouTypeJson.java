...
public abstract class GeshouTypeJson<T> extends Callback<T> {
    private Class<T> clazz;
    private Gson gson;
    public GeshouTypeJson(Class<T> clazz)
    {
        this.clazz = clazz;
        gson = new Gson();
    }
    ...
    public T parseNetworkResponse(Response response, int id) throws Exception {
        try
        {
            ...
            String jsonSub = Jsonresponse.substring(Jsonresponse.indexOf("[{"),Jsonresponse.lastIndexOf("}]")+2);
            String jsonSub4 = "{"+"\"info\""+":"+jsonSub+"}";
            createjson(jsonSub4);
